...
import { Droplet_multi_create } from "../../src/dots/models/index.js";
import { Alert_policy_request } from "../../src/dots/models/index.js";
import { AlertsPostResponse } from "../../src/dots/v2/monitoring/alerts/index.js";
import { GetDirectionQueryParameterType, GetInterfaceQueryParameterType } from "../../src/dots/v2/monitoring/metrics/droplet/bandwidth/index.js";
import { Metrics } from "../../src/dots/models/index.js";
dotenv.config();
...
describe("Monitoring Integration Tests", () => {
    ...
    it("should create, list, get, update, and delete an alert policy", async () => {
        const testDropletReq: DropletRequest = {
            name: `${PREFIX}-${uuidv4()}`,
            region: REGION,
            size: DROPLET_SIZE,
            image: DROPLET_IMAGE,
        ...
        try {
            ...
            const createAlertReq: Alert_policy_request = {
                alerts: {
            ...
            try {
                ...
                const updateAlertReq: Alert_policy_request = {
                    alerts: {
                        email: ["mchittupolu@digitalocean.com"],
                    },
                    compare: "GreaterThan",
    ...
    it("should get various metrics", async () => {
        ...
        try {
            ...
            const bandwidthMetric = await getDropletBandwidthMetrics(
                dropletId.toString(),
                "public",
                "outbound",
                startTime.toString(),
            ...
            const cpuMetric = await getDropletCpuMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(cpuMetric.status).toBe("success");
            const filesystemFreeMetric = await getDropletFilesystemFreeMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(filesystemFreeMetric.status).toBe("success");
            const load1Metric = await getDropletLoad1Metrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(load1Metric.status).toBe("success");
            const load5Metric = await getDropletLoad5Metrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(load5Metric.status).toBe("success");
            const load15Metric = await getDropletLoad15Metrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(load15Metric.status).toBe("success");
            const memoryCachedMetric = await getDropletMemoryCachedMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(memoryCachedMetric.status).toBe("success");
            const memoryFreeMetric = await getDropletMemoryFreeMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(memoryFreeMetric.status).toBe("success");
            const memoryTotalMetric = await getDropletMemoryTotalMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
            expect(memoryTotalMetric.status).toBe("success");
            const memoryAvailableMetric = await getDropletMemoryAvailableMetrics(
                dropletId.toString(),
                startTime.toString(),
                endTime.toString()
            );
    ...
    async function createDroplet(req: Droplet_multi_create): Promise<Droplet> {
    ...
    async function waitForDropletNetworks(dropletId: number): Promise<Droplet> {
        let attempts = 0;
    ...
    async function getDroplet(dropletId: number): Promise<Droplet> {
        try {
            const resp = await client.v2.droplets.byDroplet_id(dropletId).get();
    ...
    async function createAlertPolicy(req: Alert_policy_request): Promise<AlertsPostResponse> {
    ...
    async function listAlertPolicies(): Promise<AlertsPostResponse[]> {
        try {
    ...
    async function getAlertPolicyById(alertUuid: string): Promise<AlertsPostResponse> {
        try {
    ...
    async function updateAlertPolicyById(alertUuid: string, req: Alert_policy_request): Promise<AlertsPostResponse> {
    ...
    async function deleteAlertPolicy(alertUuid: string): Promise<void> {
    ...
    async function getDropletBandwidthMetrics(hostId: string, interfaceType: GetInterfaceQueryParameterType, direction: GetDirectionQueryParameterType, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletCpuMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletFilesystemFreeMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletLoad1Metrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletLoad5Metrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletLoad15Metrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletMemoryCachedMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletMemoryFreeMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletMemoryTotalMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
    ...
    async function getDropletMemoryAvailableMetrics(hostId: string, start: string, end: string): Promise<Metrics> {
        try {
