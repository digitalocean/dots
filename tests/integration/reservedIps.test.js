...
describe("Reserved IPs Integration Tests", () => {
    it("should create, assign, unassign, and delete a reserved IP", async () => {
        ...
        const createDropletReq = {
            name: dropletName,
            region: REGION,
            size: DROPLET_SIZE,
            image: DROPLET_IMAGE,
        };
        const droplet = await createDroplet(createDropletReq);
        try {
            if (!droplet.id) {
                throw new Error("Droplet ID is missing");
            }
            await waitForDropletReady(droplet.id);
            const createReservedIpReq = {
                region: REGION,
            };
            ...
            try {
                ...
                const assignAction = await assignReservedIp(reservedIp.ip, droplet.id);
                ...
                const unassignAction = await unassignReservedIp(reservedIp.ip);
    ...
    async function createReservedIp(req) {
    ...
    async function assignReservedIp(ip, dropletId) {
    ...
    async function unassignReservedIp(ip) {
    ...
    async function waitForDropletReady(dropletId) {
    ...
    async function deleteReservedIp(ip) {
